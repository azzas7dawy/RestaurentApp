...
class AdminCategoryItemsScreen extends StatefulWidget {
  ...
  @override
  State<AdminCategoryItemsScreen> createState() =>
      _AdminCategoryItemsScreenState();
}
...
class _AdminCategoryItemsScreenState extends State<AdminCategoryItemsScreen> {
  ...
  Future<void> _addOrEditItem({DocumentSnapshot? existingDoc}) async {
    ...
    final nameArController =
        TextEditingController(text: existingDoc?['title_ar'] ?? '');
    final nameEnController =
        TextEditingController(text: existingDoc?['title'] ?? '');
    final descArController =
        TextEditingController(text: existingDoc?['desc_ar'] ?? '');
          ...
          child: Column(
            children: [
              TextField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Name (Arabic)')),
              TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (nameArController.text.isEmpty || priceController.text.isEmpty)
                return;
